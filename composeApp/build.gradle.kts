...
compose.desktop {
    application {
        mainClass = "ca.weblite.jdeployKmpStarter.MainKt"

        nativeDistributions {
            targetFormats(TargetFormat.Dmg, TargetFormat.Msi, TargetFormat.Deb)
            packageName = "ca.weblite.jdeployKmpStarter"
...
tasks.register<com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar>("shadowJarExecutable") {
    archiveBaseName.set("jdeploy-kmp-starter")
    archiveClassifier.set("all")
    from(kotlin.jvm().compilations.getByName("main").output)
    configurations = listOf(project.configurations.getByName("jvmRuntimeClasspath"))
    manifest {
        attributes["Main-Class"] = "ca.weblite.jdeployKmpStarter.MainKt"
    }
}
...
tasks.register("buildExecutableJar") {
    ...
    doLast {
        println("Built executable JAR: composeApp/build/libs/jdeploy-kmp-starter-all.jar")
    }
}
